 ...
 * @file    crc_check.c
 * @brief   CRC 校验模块
 * @note    提供 CRC8/CRC16 校验计算
 *          用于 I2C/SPI/UART 通信数据完整性验证
 */
...
/**
 * @brief   CRC8-Maxim 计算 (用于DS18B20等传感器)
 * @note    多项式: x^8 + x^5 + x^4 + 1 (0x31)
 */
 ...
 * @brief   验证 CRC16
 * @param   data:     数据指针
 * @param   len:      数据长度 (包含CRC字段)
 * @param   expected: 期望的CRC值
 * @return  true=校验通过, false=校验失败
 */
bool crc16_verify(uint8_t *data, uint16_t len, uint16_t expected)
{
